...
import { listTraces, type StoredTrackPoint } from "../api/backend";
import { TraceMap } from "../components/TraceMap";
import { activityForest as forest } from "./activityForestTheme";
...
export function ActivityPage({ onOpen }: { onOpen: (id: string) => void }) {
  ...
  const [latestPoints, setLatestPoints] = useState<StoredTrackPoint[]>([]);
  ...
  useEffect(() => {
    listTraces().then((items) => setLatestPoints(items.find((item) => item.points.length)?.points ?? [])).catch(console.error);
  }, []);
  ...
  return <SafeAreaView style={[styles.safeArea, forest.safeArea]}>
    <View style={styles.header}>
      <View><Text style={[styles.eyebrow, forest.eyebrow]}>YOUR MEMORY MAP</Text><Text style={[styles.title, forest.title]}>Activity</Text></View>
      <View style={styles.controls}>
        <Pressable accessibilityLabel="Filter activity" onPress={() => setShowFilter(true)} style={[styles.filterButton, forest.filterButton]}><Feather color="#E6F0D5" name="sliders" size={16} /></Pressable>
        <View style={[styles.viewSwitch, forest.viewSwitch]}>
          <Pressable accessibilityLabel="Card view" onPress={() => setMode("cards")} style={[styles.switchButton, mode === "cards" && styles.activeSwitch]}><Feather color={mode === "cards" ? "#F4F2EA" : "#BDD0BF"} name="credit-card" size={16} /></Pressable>
          <Pressable accessibilityLabel="Grid view" onPress={() => setMode("grid")} style={[styles.switchButton, mode === "grid" && styles.activeSwitch]}><Feather color={mode === "grid" ? "#F4F2EA" : "#BDD0BF"} name="grid" size={16} /></Pressable>
        </View>
      </View>
    </View>
    <View style={styles.subheader}><Text style={[styles.subheaderText, forest.subheaderText]}>{mode === "cards" ? "Swipe through a trace at a time" : "Photo memories · newest first"}</Text><Text style={[styles.count, forest.count]}>{visibleTraces.length} TRACES</Text></View>
    {latestPoints.length ? <View style={styles.latestMap}><TraceMap points={latestPoints} /></View> : null}

    {visibleTraces.length === 0 ? <View style={styles.empty}><Feather color="#BFD8A6" name="search" size={22} /><Text style={[styles.emptyTitle, forest.emptyTitle]}>No traces here yet.</Text><Text style={[styles.emptyCopy, forest.emptyCopy]}>Try a wider month range or another category.</Text></View> : mode === "cards" ? <ScrollView contentContainerStyle={[styles.cardScroller, forest.cardScroller]} decelerationRate="fast" horizontal pagingEnabled showsHorizontalScrollIndicator={false} snapToInterval={cardWidth + 12}>
      {visibleTraces.slice(0, 5).map((trace) => <TraceCard cardWidth={cardWidth} key={trace.id} onOpen={openTrace} trace={trace} />)}
    </ScrollView> : <ScrollView contentContainerStyle={[styles.gridScroller, forest.gridScroller]} showsVerticalScrollIndicator={false}>
...
function TraceCard({ cardWidth, onOpen, trace }: { cardWidth: number; onOpen: (id: string) => void; trace: Trace }) {
  return <Pressable onPress={() => onOpen(trace.id)} style={[styles.card, forest.card, { width: cardWidth }]}>
    <ImageBackground imageStyle={styles.mapImage} source={mapSource} style={[styles.cardMap, forest.cardMap]}><View style={styles.mapTint} /><View style={styles.traceLine} /><View style={styles.marker}><Feather color="#F4F2EA" name="map-pin" size={17} /></View><View style={styles.mapTag}><Feather color="#D8E9C2" name="navigation" size={12} /><Text style={styles.mapTagText}>TRACE</Text></View></ImageBackground>
    <View style={[styles.cardBody, forest.cardBody]}><Text style={[styles.cardDate, forest.cardDate]}>{trace.time.toUpperCase()}</Text><Text style={[styles.cardTitle, forest.cardTitle, forest.cardTitleColor]}>{trace.title}</Text><View style={[styles.cardFooter, forest.cardFooter]}><Text style={[styles.cardMeta, forest.cardMeta]}>{trace.echoes} · {trace.distance}</Text><View style={styles.open}><Text style={[styles.openText, forest.openText]}>OPEN</Text><Feather color="#174B3A" name="arrow-up-right" size={14} /></View></View></View>
  </Pressable>;
}
